...
struct SDL_StorageInterface{
	...
	private extern(C) nothrow{
		...
		alias ReadFileFn = bool function(void* userData, const(char)* path, void* destination, c_uint64 length);
		alias WriteFileFn = bool function(void* userData, const(char)* path, const(void)* source, c_uint64 length);
		...
		alias SpaceRemainingFn = c_uint64 function(void* userData);
	}
	CloseFn close;
	ReadyFn ready;
...
mixin(joinFnBinds((){
	FnBind[] ret = [
		...
		{q{bool}, q{SDL_GetStorageFileSize}, q{SDL_Storage* storage, const(char)* path, c_uint64* length}},
		{q{bool}, q{SDL_ReadStorageFile}, q{SDL_Storage* storage, const(char)* path, void* destination, c_uint64 length}},
		{q{bool}, q{SDL_WriteStorageFile}, q{SDL_Storage* storage, const(char)* path, const(void)* source, c_uint64 length}},
		...
		{q{c_uint64}, q{SDL_GetStorageSpaceRemaining}, q{SDL_Storage* storage}},
