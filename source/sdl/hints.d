...
enum{
	...
	SDL_HINT_JOYSTICK_ZERO_CENTRED_DEVICES = SDL_HINT_JOYSTICK_ZERO_CENTERED_DEVICES,
	...
	SDL_HINT_MOUSE_RELATIVE_MODE_CENTRE = SDL_HINT_MOUSE_RELATIVE_MODE_CENTER,
...
mixin(makeEnumBind(q{SDL_HintPriority}, aliases: [q{SDL_Hint}], members: (){
	EnumMember[] ret = [
...
mixin(joinFnBinds((){
	FnBind[] ret = [
		{q{bool}, q{SDL_SetHintWithPriority}, q{const(char)* name, const(char)* value, SDL_HintPriority priority}},
		{q{bool}, q{SDL_SetHint}, q{const(char)* name, const(char)* value}},
		{q{bool}, q{SDL_ResetHint}, q{const(char)* name}},
		{q{void}, q{SDL_ResetHints}, q{}},
		{q{const(char)*}, q{SDL_GetHint}, q{const(char)* name}},
		{q{bool}, q{SDL_GetHintBoolean}, q{const(char)* name, bool defaultValue}},
		{q{bool}, q{SDL_AddHintCallback}, q{const(char)* name, SDL_HintCallback callback, void* userData}},
		{q{void}, q{SDL_RemoveHintCallback}, q{const(char)* name, SDL_HintCallback callback, void* userData}},
	];
	return ret;
}()));
