...
SDL_AppResult SDL_AppIterate(void* appState){
	...
	dst_rect.w = cast(float)textureWidth + (textureWidth * scale);
	dst_rect.h = cast(float)textureHeight + (textureHeight * scale);
	dst_rect.x = (cast(float)(WINDOW_WIDTH - dst_rect.w)) / 2.0f;
	dst_rect.y = (cast(float)(WINDOW_HEIGHT - dst_rect.h)) / 2.0f;
